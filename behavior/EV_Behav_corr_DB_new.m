...
%   Known expe         'Explo', 'CondMov', 'FullTask', 'RipplesEpoch'
%
% EXAMPLE
%
...
%% Calculate EV
[EV, ~, states_sleep, states_wake, num_mice] = ExplainedVariance_master_DB(mice, expe, 'PlotResults', false, 'IsII', false, 'SplitSleep', []);
...
EntryTime_shock = nan(length(a), 4, 2); % 4 tests, Pre and Post
for i = 1:length(a)
    for k=1:length(id_Pre{i})
...
if sav
    ...
    saveas(f,[foldertosave filesep 'EV' filesep 'Correlations' filesep 'CorrEV_' type_sleep '_' EV_epoch '_' expe '.fig']);
    saveFigure(f,['CorrEV_' type_sleep '_' EV_epoch '_' expe], [foldertosave filesep 'EV' filesep 'Correlations']);
end
