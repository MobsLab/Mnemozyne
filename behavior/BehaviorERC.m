...
elseif strcmp(experiment, 'Known')
    fetchpaths = 'Known';   
elseif strcmp(experiment, 'Sub')
    fetchpaths = 'Sub';
end

%% Get data
if ~strcmp(fetchpaths,'Sub')
    Dir = PathForExperimentsERC(fetchpaths);
else
    Dir = PathForExperiments_TC(fetchpaths);
end

if ~ (strcmp(nMice, 'all') | strcmp(nMice, 'All'))
...
for i=1:length(a)
    for k=1:length(id_Pre{i})
        if isempty(a{i}.behavResources(id_Pre{i}(k)).Xtsd)
            a{i}.behavResources(id_Pre{i}(k)).Xtsd = a{i}.behavResources(id_Pre{i}(k)).AlignedXtsd;
        end
        if isempty(a{i}.behavResources(id_Pre{i}(k)).Ytsd)
            a{i}.behavResources(id_Pre{i}(k)).Ytsd = a{i}.behavResources(id_Pre{i}(k)).AlignedYtsd;
        end
        ...
        if isempty(a{i}.behavResources(id_Post{i}(k)).Xtsd)
            a{i}.behavResources(id_Post{i}(k)).Xtsd = a{i}.behavResources(id_Post{i}(k)).AlignedXtsd;
        end
        if isempty(a{i}.behavResources(id_Post{i}(k)).Ytsd)
            a{i}.behavResources(id_Post{i}(k)).Ytsd = a{i}.behavResources(id_Post{i}(k)).AlignedYtsd;
        end
...
if IsSave
    ...
    saveFigure(fh.Number, [FigName '_' experiment name_addon], foldertosave);
end
...
if IsSave
    ...
    saveFigure(f1.Number, [FigName '_diff' '_' experiment name_addon], foldertosave);
end
...
if IsSave
    ...
    saveFigure(f2.Number, [FigName '_neutral' '_' experiment name_addon], foldertosave);
end
