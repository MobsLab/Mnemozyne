function [sEpoch subst] = get_SleepEpoch_sess(dirPath,Vtsd)
...
% load sleep scoring
try
    load([dirPath 'SleepSubstages.mat'],'Epoch','Epoch_sess');
    if exist('Epoch_sess','var')
        Epoch = Epoch_sess;
        sEpoch{3,1} = or(Epoch{1}{7},Epoch{2}{7});        % nrem
        sEpoch{3,2} = or(Epoch{1}{4},Epoch{2}{4});        % rem
        sEpoch{3,3} = or(Epoch{1}{5},Epoch{2}{5});        % wake
        sEpoch{3,4} = or(Epoch{1}{1},Epoch{2}{1});        % n1
        sEpoch{3,5} = or(Epoch{1}{2},Epoch{2}{2});        % n2
        sEpoch{3,6} = or(Epoch{1}{3},Epoch{2}{3});        % n3
        sEpoch{3,7} = or(or(Epoch{1}{4},Epoch{1}{7}), ...
                        or(Epoch{2}{4},Epoch{2}{7}));      % sleep  
    else
