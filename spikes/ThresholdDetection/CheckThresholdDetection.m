...
%% Get delays
[L, NotFound, NotFound_idx] = GetLatencyThreshold(rawchan, StimEpoch, ThresholdUsed);
...
title(['Detection-Stimulation delays (N=' num2str(sum(~isnan(L_toplot))) '/' num2str(length(L)) ')'])
xlabel('Time (ms)')
ylabel('Count')
box off
...
%% Found/Unfound spikes
FoundPerc = sum(~isnan(L))/length(L)*100;
NotPoundPerc = 100 - FoundPerc;


f1 = figure('rend','painters','pos',[1 1 650 400]);
a = [FoundPerc; NotPoundPerc];
h = pie(a);
h(1).FaceColor = [0 1 0];
h(3).FaceColor = [1 0 0];
legend('Detected spikes', 'Stims without a spike', 'Location', [0.77 0.8 0.1 .1])
title(['Detection rate (N=' num2str(length(L)) ' stims)'])
